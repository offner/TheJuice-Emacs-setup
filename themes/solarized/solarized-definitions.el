...
(defun solarized-faces (facespecs)
  (mapcar (lambda (facespec-with-name)
            (let* ((name (car facespec-with-name))
                   ...
                   (facespec-tty-256 (solarized-face-for-index facespec 1))
                   (facespec-tty-term (solarized-face-for-index facespec 1))
                   (facespec-default (solarized-face-for-index facespec 1)))
              `(,name
     ...
     (append
      (solarized-faces
       `( ;; basic
         ...
         (cursor (:foreground ,foreground-grey :background yellow :inverse-video t))
